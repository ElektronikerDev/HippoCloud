...
public class ProxyGroupSetupConsole implements Setup, SimpleFileWriter {
    ...
    public String getName() {
        return "ProxyGroupSetup";
    }

    @Override
    public void hello() {
        log("Start ProxyGroupSetup-Setup:");
        getNewQuestion();
    }

    @Override
    public void handle(String[] args) {
        if (setupStepsClass == null) setupStepsClass = new ProxyGroupSetupSteps().getClass();
    ...
    public void finishedSetup() {
        try {
            writeToFile(new File("./local/groups/proxys/" + getAnswer("name") + "/"), new Gson().toJson(setupAnswers), false);
        } catch (Exception e) {
