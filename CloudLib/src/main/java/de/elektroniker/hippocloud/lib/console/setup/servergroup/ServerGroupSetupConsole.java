package de.elektroniker.hippocloud.lib.console.setup.servergroup;
...
public class ServerGroupSetupConsole implements Setup, SimpleFileWriter {
    ...
    public String getName() {
        return "ServerGroupSetup";
    }

    @Override
    public void hello() {
        log("Start ServerGroup-Setup:");
        getNewQuestion();
    }

    @Override
    public void handle(String[] args) {
        if(setupStepsClass == null)setupStepsClass = new ServerGroupSetupSteps().getClass();
    ...
    public void finishedSetup() {
        try {
            writeToFile(new File("./local/groups/servers/" + getAnswer("name") +".json"), new Gson().toJson(setupAnswers), false);
        } catch (Exception e) {
